...
function showView(viewId) {
  ...
  if (viewId === 'study') startStudySession();
...
function renderDashboard() {
  ...
  const catDue = {};
  dueCards.forEach(c => { catDue[c.category] = (catDue[c.category] || 0) + 1; });
    ...
    upList.innerHTML = upcoming.map(c => {
      ...
      return `
        <div class="upcoming-item">
          <span class="upcoming-question">${c.front}</span>
...
function renderBrowse() {
  ...
  const cats = [...new Set(state.cards.map(c => c.category))].sort();
  const currentCat = filterCatEl.value;
  ...
  grid.innerHTML = filtered.map(card => {
    ...
    return `
      <div class="card-item">
        <div class="card-item-category">${card.category}</div>
...
function handleFormSubmit(e) {
  ...
  const data = {
    ...
    front: document.getElementById('form-front').value.trim(),
    back: document.getElementById('form-back').value.trim(),
    hint: document.getElementById('form-hint').value.trim(),
    explanation: document.getElementById('form-explanation').value.trim()
  };

  if (!data.front || !data.back || !data.category) {
...
function editCard(id) {
  ...
  document.getElementById('form-explanation').value = card.explanation || '';
}
